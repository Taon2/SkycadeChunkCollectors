...
public class OresGui extends DynamicGui {
    ...
    public OresGui(BlockData blockData, UUID uuid, int page) {
        super(ChatColor.AQUA + "" + ChatColor.BOLD + "Ores", 6);
        blockData.addStorageViewer(uuid);

        setCloseInteraction((p, ev) -> blockData.removeStorageViewer(uuid));

        ChunkCollectorManager.getOres().stream()
                .skip((page - 1) * 45)
                .limit(45)
                .forEach(material -> addItemInteraction(p -> {
                        ...
                        (p, ev) -> {
                            ...
                            new OresGui(blockData, uuid, page).open(p);
                        }));
